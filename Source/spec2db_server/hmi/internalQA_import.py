...
class InternalQAImport(HmiInternalQA):
    def __init__(self):
        HmiInternalQA.__init__(self)

    def do_import(self, xls_file_name, commit_user_id, update_time):
        result = {"result": "NG", "error": ''}
        try:
            xls_list = DocXls('HMI_INTERNALQA', '0.01').load_data(xls_file_name)
        ...
        try:
            ...
            result["error"] = "%s%s" % (new_data_dict.get('qa_no'), str(e))
        finally:
            self._pg.close()
            return result
...
if __name__ == '__main__':
    ...
    InternalQAImport().do_import(r'/mnt/sharedoc/HMI_InternalQA_v001.xlsx', 341,
