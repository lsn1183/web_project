...
class ExternalQAImport(HmiExternalQA):
    def __init__(self):
        HmiExternalQA.__init__(self)

    def do_import(self, xls_file_name, commit_user_id, update_time):
        result = {"result": "NG", "error": ''}
        try:
            xls_list = DocXls('HMI_EXTERNALQA', '0.02').load_data(xls_file_name)
        ...
        try:
            ...
            result["error"] = "%s%s" % (new_data_dict.get('qa_keyword'), str(e))
        finally:
            self._pg.close()
            return result
...
if __name__ == '__main__':
    ...
    ExternalQAImport().do_import(r'/mnt/sharedoc/HMI_ExternalQA_v001.xlsx', 341,
