...
class ExternalBugImport(HmiExternalBug):
    def __init__(self):
        HmiExternalBug.__init__(self)

    def do_import(self, xls_file_name, commit_user_id, update_time):
        result = {"result": "NG", "error": ''}
        try:
            xls_list = DocXls('HMI_EXTERNALBUG', '0.02').load_data(xls_file_name)
        ...
        try:
            ...
            result["error"] = "%s%s" % (new_data_dict.get('bug_id'), str(e))
        finally:
            self._pg.close()
            return result
...
if __name__ == '__main__':
    ...
    ExternalBugImport().do_import(r'/mnt/sharedoc/HMI_ExternalQA_v001.xlsx', 341,
