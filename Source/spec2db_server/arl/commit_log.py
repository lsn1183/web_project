...
class CommitLog(ServiceBase):
    ...
    def _create_commit_log_table(self, ):
        sqlcmd = '''
            -- DROP TABLE IF EXISTS spec.commit_log;
            CREATE TABLE spec.commit_log
            (
            ...
            ALTER TABLE spec.commit_log
            OWNER TO postgres;
            
            CREATE INDEX COMMIT_LOG_COMMIT_ID ON spec.commit_log(commit_id);
            CREATE INDEX COMMIT_LOG_USER_ID ON spec.commit_log(user_id);
            CREATE INDEX COMMIT_LOG_GROUP_ID ON spec.commit_log(group_id);
            CREATE INDEX COMMIT_LOG_COMMIT_TIME ON spec.commit_log(commit_time);
            
            
            -- DROP TABLE IF EXISTS spec.commit_log_ref;
            CREATE TABLE spec.commit_log_ref(
                ...
                CONSTRAINT commit_log_commit_id_ref_fkey FOREIGN KEY (commit_id)
                  REFERENCES spec.commit_log (commit_id) MATCH SIMPLE
                  ON UPDATE RESTRICT ON DELETE RESTRICT
            )
            WITH (
            OIDS=FALSE
            );
            ALTER TABLE spec.commit_log_ref
            OWNER TO postgres;
            CREATE INDEX COMMIT_LOG_REF_commit_id ON spec.commit_log_ref(commit_id);
            CREATE INDEX COMMIT_LOG_REF_CLASSIFY ON spec.commit_log_ref(classify);
            
            
            -- DROP TABLE IF EXISTS spec.commit_log_info;
            CREATE TABLE spec.commit_log_info(
                ...
                CONSTRAINT commit_log_commit_log_ref_id_ref_fkey FOREIGN KEY (commit_log_ref_id)
                  REFERENCES spec.commit_log_ref (commit_log_ref_id) MATCH SIMPLE
                  ON UPDATE RESTRICT ON DELETE RESTRICT
            )
            WITH (
            OIDS=FALSE
            );
            ALTER TABLE spec.commit_log_info
            OWNER TO postgres;
            
            CREATE INDEX COMMIT_LOG_INFO_COMMIT_LOG_REF_ID ON spec.commit_log_info(commit_log_ref_id);
            CREATE INDEX COMMIT_LOG_INFO_RECORD_ID ON spec.commit_log_info(record_id);
        '''
    ...
    def _get_last_commit_by_record_id(self, pg, record_id):
        ...
        sql_cmd = '''WITH r AS (
              SELECT max(commit_log_info_id) AS commit_log_info_id FROM spec.commit_log_info WHERE record_id = %s
            )
            SELECT a.column_info FROM spec.commit_log_info AS a 
            JOIN r ON r.commit_log_info_id = a.commit_log_info_id;''' % record_id
        pg.execute(sql_cmd)
        rows = pg.fetchall()
        if len(rows) == 0:
            # log...
            ...
            for table in data['commit_tables']:
                ...
                for record in table['records']:
                    ...
                    if '1' not in status_flag and str(action_type) == '1':
                        continue
                    sql_cmd = '''INSERT INTO spec.commit_log_info(commit_log_ref_id, record_id, action_type, status_flag, column_info)
                                 VALUES(%s, %s, %s, b%s, %s);
                    '''
    ...
    def _add_commit_log_ref(self, pg, data, commit_id):
        ...
        sql_cmd = '''
        INSERT INTO spec.commit_log_ref(commit_id, table_id, classify) 
    ...
    def get_commit_log_by_user(self, user_id, s_time, e_time):
        ...
        sql_cmd = '''SELECT commit_id, lower(b.user_name), lower(c.group_name), commit_time
                     FROM spec.commit_log AS a
                     JOIN spec.arl_user AS b ON a.user_id = b.user_id
                     JOIN spec.arl_group AS c ON a.group_id = c.group_id
        ...
        for row in rows:
            ...
            sql_cmd = '''SELECT DISTINCT classify FROM spec.commit_log_ref WHERE commit_id = %s;'''
    ...
    def get_commit_log(self, s_time, e_time, group_id=0, user_id=0, size=0, page=0):
        ...
        FROM spec.commit_log AS a
        left JOIN spec.arl_user AS b
        ON a.user_id = b.user_id
        left JOIN spec.arl_group AS c
    ...
    def _get_commit_log_brief(self, pg, commit_id, size, page):
        ...
        SELECT t2.commit_log_ref_id, table_name, column_info
          ...
          left join spec.commit_log_info as t2
    ...
    def get_commit_log_by_classify_and_record_id(self, classify, r_id):
        ...
        SELECT t3.commit_id, t3.user_id, t4.user_name, 
          ...
          FROM spec.commit_log_info AS t1
          LEFT JOIN spec.commit_log_ref as t2
          on t1.commit_log_ref_id = t2.commit_log_ref_id
          LEFT JOIN spec.commit_log AS t3
          on t2.commit_id = t3.commit_id
          LEFT JOIN spec.arl_user AS t4
          ON t3.user_id = t4.user_id
          LEFT JOIN spec.arl_group AS t5
    ...
    def _commit_log_info(self, pg, commit_log_ref_id, record_id,
                         ...
                         reason):
        sql_cmd = '''
        INSERT INTO spec.commit_log_info(commit_log_ref_id, record_id, action_type,
    ...
    def _get_log_info(self, pg, commit_log_ref_id):
        ...
        SELECT commit_id, t1.commit_log_ref_id, t3.table_name, classify, 
          ...
          FROM spec.commit_log_ref AS t1
          LEFT JOIN spec.commit_log_info AS t2
          ON t1.commit_log_ref_id = t2.commit_log_ref_id
          LEFT JOIN spec.log_table_map as t3
    ...
    def _get_checklist_brief(self, pg, start_time, end_time):
        ...
        for table_info in table_infos:
              ...
              FROM (
                  ...
                  FROM (
                        SELECT commit_id, user_id, group_id, commit_time
                          FROM spec.commit_log
                          where commit_time >= %s
                                and commit_time <= %s
                  ) as t1
                  left join spec.commit_log_ref as t2
                  on t1.commit_id = t2.commit_id
                  left join spec.commit_log_info as t3
                  on t2.commit_log_ref_id = t3.commit_log_ref_id
                  left join spec.{req_table} as t4
            ...
            """.format(req_table=table_info.get("table_name"),
    ...
    def get_cimmit_log_by_id(self, commit_id):
        sqlcmd = """
            ...
            FROM spec.commit_log AS t1 LEFT JOIN spec.arl_user AS t2 ON t1.user_id = t2.user_id
