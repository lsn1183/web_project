...
colors = {"equal": '%s',
          "insert": '<span style="background:#e6ffe6">%s</span>',  # <!-- 新增文本 -->
          "replace": '<span style="background:#f56c6c">%s</span>',  # <!-- 修改文本 -->
          "delete": '<span style="background:#ffe6e6; text-decoration:line-through">%s</span>',  # <!-- 删除文本 -->
         }


class MySequenceMatcher(difflib.SequenceMatcher):
    def get_color_strs(self):
        ...
        for tag, i1, i2, j1, j2 in self.get_opcodes():
            ...
            if tag == 'equal':
                ...
                str2 += colors.get(tag) % self.b[j1:j2]
            elif tag == "replace":
                str1 += colors.get(tag) % self.a[i1:i2]
                str2 += colors.get(tag) % self.b[j1:j2]
            elif tag == "delete":
                str1 += colors.get(tag) % self.a[i1:i2]
        return str1, str2
...
# a = u"中国bbcc"
# b = u"中abccaaa"
# s = MySequenceMatcher(None, a, b)
# for tag, i1, i2, j1, j2 in s.get_opcodes():
#     print("%7s a[%d:%d] (%s) b[%d:%d] (%s)" %
#           (tag, i1, i2, a[i1:i2], j1, j2, b[j1:j2]))
# str1, str2 = s.get_color_strs()
# print(str1)
# print(str2)
...
class mydiff_match_patch(dmp_module.diff_match_patch):
    def __init__(self):
        dmp_module.diff_match_patch.__init__(self)

    def diff_prettyHtml1(self, diffs):
