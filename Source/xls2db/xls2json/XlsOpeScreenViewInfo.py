...
class XlsOpeScreenViewInfo:
    ...
    def make_screen_view_info(self, content_list):
        screen_view_dict = {}
        screen_view_dict["title_info"] = "View of Screen"

        view_content_e_row = self.make_reference_info(screen_view_dict)
        ...
        while one_part_s_row > 0 and one_part_e_row > 0:
            ...
            screen_view_dict["content_list"].append(one_part_dict)

        content_list.append(screen_view_dict)
    ...
    def make_one_part_info(self, part_s_row, part_e_row, one_part_dict):
        ...
        view_img_s_row = part_s_row+1

        if view_img_s_row == part_e_row:
            return

        view_img_e_row = part_e_row
        for ir in range(view_img_s_row, part_e_row):
            if XlsUtil().get_cell_str_value(ope_sheet, ir, 3):
                break
            view_img_e_row = ir

        img_file_name = "%sR%d.jpg" % (self.ope_no, view_img_s_row)
        #print view_img_s_row, view_img_e_row
        if XlsUtil().cap_range_as_img(ope_sheet, self.img_dir, img_file_name, view_img_s_row,
                                      4, view_img_e_row, 49) == True:
            one_part_dict["viewimg_info"] = ('''%s/%s''') % (self.img_dir.split("\\")[-1], img_file_name)

        action_info_s_row = view_img_e_row
        if action_info_s_row+1 >= part_e_row:
            return
