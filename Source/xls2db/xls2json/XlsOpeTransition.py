...
class XlsOpeTransition:
    ...
    def get_row_range(self):
        ...
        for ir in range(self.start_row, self.end_row + 1):
            ...
            if line_str[0].lower() == "action on transition":
                self.start_row = ir
    ...
    def make_transition_info(self, content_list):
        ope_sheet = self.ope_sheet

        transition_info_dict = {}
        transition_info_dict["title_info"] = "Action on Transition"
        ...
        if XlsUtil().cap_range_as_img(ope_sheet, self.img_dir, img_file_name, self.start_row+1,
            ...
            transition_info_dict["action_info"] = ('''%s/%s''') % (self.img_dir.split("\\")[-1], img_file_name)

        content_list.append(transition_info_dict)
