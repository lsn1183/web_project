...
Revision ID: 0a0825157942
Revises: e8b6de1e41df
Create Date: 2019-03-08 16:50:52.785242
...
revision = '0a0825157942'
down_revision = 'e8b6de1e41df'
branch_labels = None
depends_on = None
...
def upgrade():
    ...
    op.add_column('option_combination', sa.Column('checked', sa.Boolean(), nullable=True), schema='func')
    op.add_column('option_combination', sa.Column('proj_id', sa.Integer(), nullable=True), schema='func')
    op.create_index(op.f('ix_func_option_combination_checked'), 'option_combination', ['checked'], unique=False, schema='func')
    op.create_index(op.f('ix_func_option_combination_proj_id'), 'option_combination', ['proj_id'], unique=False, schema='func')
    op.create_foreign_key(None, 'option_combination', 'projects', ['proj_id'], ['proj_id'], source_schema='func', referent_schema='func')
...
def downgrade():
    ...
    op.drop_constraint(None, 'option_combination', schema='func', type_='foreignkey')
    op.drop_index(op.f('ix_func_option_combination_proj_id'), table_name='option_combination', schema='func')
    op.drop_index(op.f('ix_func_option_combination_checked'), table_name='option_combination', schema='func')
    op.drop_column('option_combination', 'proj_id', schema='func')
    op.drop_column('option_combination', 'checked', schema='func')
