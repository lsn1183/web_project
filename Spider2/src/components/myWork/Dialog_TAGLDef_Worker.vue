						...
						<div class="s_tagldef_table">
							...
							<div class='s_table_content' style="width:33%">
								...
								<p class="s_table_textarea">
									<el-input type="textarea" :autosize="{minRows:5, maxRows:5}" resize='none' v-model="seq_info.status" :disabled="tagl_def_reponsible_select_list[index]!=undefined
											&&tagl_def_reponsible_select_list[index][0]=='TAGL-PF'?false:true"></el-input>
								</p>
							</div>
							<div class='s_table_content' style="width:34%">
								...
								<p class="s_table_textarea">
									<el-input type="textarea" :autosize="{minRows:5, maxRows:5}" resize='none' v-model="seq_info.trigger" :disabled="tagl_def_reponsible_select_list[index]!=undefined
											&&tagl_def_reponsible_select_list[index][0]=='TAGL-PF'?false:true"></el-input>
								</p>
							</div>
							<div class='s_table_content' style="width:33%">
	...
	export default {
		...
		methods: {
			onshow() {
				...
				this.tagl_def_seq_list = this.tagl_def_data_copy.sequence_list;
				this.tagl_def_opened_data = JSON.stringify(this.tagl_def_data_copy.sequence_list)
				...
				this.tagl_def_all_responsible_list.push(hu_dict);
				this.tagl_def_model_list_id_select_dict = {}

				this.$axios.get(this.Ip + '/ForestModelTypes/definition')
					.then(res => {
				...
				for(let model_dict of source_list) {
					...
					if(model_dict.sub_list.length == 0) {
						...
						if('info' in model_dict) {
							this.tagl_def_model_list_id_name_dict[model_dict['model_id']] = {
			...
			tagl_def_init_reponsible_select_list() {
				for(let hu_data of this.tagl_def_seq_list) {
					...
					if(hu_data.type == "DEVICE") {
						...
						tmp_responsible_list.push('TAGL-PF')
						tmp_responsible_list.push(hu_data.type)
						this.tagl_def_reponsible_select_list.push(tmp_responsible_list)
					}
				}
			},
			...
			close() {
				this.tagl_def_closed_data = JSON.stringify(this.tagl_def_data_copy.sequence_list)
			...
			save() {
				for(let i = 0; i < this.tagl_def_seq_list.length; i++){
					var getStatus = false;
					if(this.tagl_def_reponsible_select_list[i][0] == 'TAGL-PF') {
						this.tagl_def_seq_list[i].type = this.tagl_def_reponsible_select_list[i][this.tagl_def_reponsible_select_list[i].length - 1];
						this.tagl_def_seq_list[i].name = this.tagl_def_reponsible_select_list[i][this.tagl_def_reponsible_select_list[i].length - 1];
						if(this.SpiderCheck.check_data(this.SpiderCheck.CHECK_COMMON_RESPOINSIBLE_STATUS, this.tagl_def_seq_list[i].status) == false) {
							getStatus = getStatus || true
						}

						if(this.SpiderCheck.check_data(this.SpiderCheck.CHECK_COMMON_RESPOINSIBLE_TRIGGER, this.tagl_def_seq_list[i].trigger) == false) {
							getStatus = getStatus || true
						}

						if(this.SpiderCheck.check_data(this.SpiderCheck.CHECK_COMMON_RESPOINSIBLE_ACTION, this.tagl_def_seq_list[i].action) == false) {
							getStatus = getStatus || true
						}
					}
					if(getStatus == false){
				...
				for(let i = 0; i < this.tagl_def_seq_list.length; i++) {
					if(this.tagl_def_reponsible_select_list[i].length < 2) {
						this.$alert('请选择責務分担！');
						return;
					}
			...
			accordion(table_id, index, definition_index) {
				if(table_id == 1) {
					$(".s_tagldef_table").eq(index).toggle(function() {})
				}
			},
		},
