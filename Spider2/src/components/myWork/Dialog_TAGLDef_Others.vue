		...
		<div class="g_dialog g_dialog_ex">
			...
			<div class="g_dialog_body g_dialog_body_ex">
				<p class="s_section">
					...
					<el-input class="s_right s_textarea" v-model="tagl_def_data_copy.author_name" disabled></el-input>			    	  	    		
	  	    	</p> 
	  	    	<p class="s_section">
	  	    		<span class="s_left">リファレンスハード上での実現可否</span>	
					<el-select class="s_right s_select" v-model="tagl_def_data_copy.implementation" placeholder="请选择" size='small'>
					    <el-option v-for="item in DONE" :key="item.value" :label="item.label" :value="item.value" >
					      <span>{{ item.label}}</span>							      
					    </el-option>
					</el-select>
	  	    	</p>	
	  	    	<p class="s_section">
	  	    		<span class="s_left">詳細分析可否</span>	
					<el-select class="s_right s_select" v-model="tagl_def_data.analysis" disabled size='small'>
					    <el-option :key="tagl_def_data.analysis" :label="tagl_def_data.analysis" :value="tagl_def_data_copy.analysis"></el-option>
					</el-select>
	  	    	</p>
    	  	    <p class="s_section">
	  	    		<span class="s_left">未要件分析</span>	
					<el-select class="s_right s_select" v-model="tagl_def_data_copy.unrequire" size='small'>
					    <el-option v-for="item in NO_ANALYSIS" :key="item.value" :label="item.label" :value="item.value"></el-option>
					</el-select>
	  	    	</p>    
	  	    	<p class="s_section">
	  	    		<span class="s_left">更新日</span>
	  	    		<el-date-picker class="s_right s_textarea" type="date" placeholder="选择日期" v-model="tagl_def_data_copy.new_date" format @change="setDate"></el-date-picker>		    	  	    		
	  	    	</p> 
	  	    	<p class="s_section">
	  	    		<span class="s_left">除外</span>
					<el-input class="s_right s_textarea" v-model="tagl_def_data_copy.exception"></el-input>			    	  	    		
	  	    	</p> 
    	    </div>
...
<script>
	export default{
		...
		methods: {
			...
			setDate(val){
				this.tagl_def_data_copy.new_date = val
			}
		},
...
/*.g_dialog_body .s_section .s_select_ex{	
	vertical-align: middle;
}*/
