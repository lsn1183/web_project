	...
	<div class="g_dialog_shadow" v-show="show">
			<div class="g_dialog g_dialog_ex">
				...
				<h2 class="g_dialog_header">
					<span class="s_section_left">シーケンス図</span>
					<!-- 点x关闭弹出框 -->
					<i class="s_section_right s_close el-icon-close" @click="close()"></i>
				</h2>
				<div class="g_dialog_body">	    	  	    	    	  	    
    	  	    	<div class="s_section">
						<el-input type="textarea" :autosize="{minRows:5, maxRows:5}" v-model="tagl_ana_data_copy.seq_diagram" style="font-size: 12px;"></el-input>
					</div> 	    	  	    	
	    	    </div>
	...
	export default{
		...
		methods: {
			onshow() {
				...
				this.show = true;
				this.tagl_ana_opened_data = this.tagl_ana_data_copy.seq_diagram
			},
			close() {
				this.tagl_ana_closed_data = this.tagl_ana_data_copy.seq_diagram
