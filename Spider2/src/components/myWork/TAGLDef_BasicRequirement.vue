  ...
  <div id="ARLlist">
        ...
        <li class="li_scroll">
          <ul class="ul_scroll">
            <li class="li" v-for="(item,index) in table1"
                @click.stop='change_up(item.definition_id,index)' :key="index">
              <i class="Asa_i" :title="item.definition_id"></i>
              <span class="cy span_width"
                    @click.stop='change_up(item.definition_id,index)'
                    :class="{'active':num == index}" :title="item.definition_id">
								<span style="marginLeft:42px">{{item.definition_id}}</span>
							</span>
              ...
              <span class="cz cen" @click="amint(item.definition_id,index)"><i class="el-icon-d-arrow-left"> 详细</i></span>
            </li>
          </ul>
        </li>
      </ul>
    ...
    <div id="list" v-show="show_flag">
      <div id="Maplist">
        ...
        <el-collapse accordion v-for="(definition_record,definition_index) in tabledata1" :key="definition_index">
              ...
              <div class="tagl_table">
                ...
                <div class='table_content' style="width:8%;border:0 none">
                  ...
                  <p class="table_msg" title="">
                    <el-button type="text" class="table_move" @click="editClick2(definition_record.definition_id,definition_index)" disabled>
                ...
                <div id="taglana_table">
                  <div class="ana_table">
                    ...
                    <div class='table_content' style="width:7%;border:0 none">
                      ...
                      <p class="table_msg" title="">
                        <el-button type="text" class="table_move" @click="editClick3(definition_record.definition_id,definition_index)" disabled>
                          <i class="el-icon-edit"> 更新</i></el-button>
                      </p>
                    </div>
                  </div>
...
<script>
  import dialog_basicreq_tagldef from './dialog_basicreq_tagldef.vue'

  require('../../assets/js/jquery-1.8.3.min.js')
  export default {
    components: {
      hu_dialog: dialog_basicreq_tagldef
    },
    data() {
      return {
        change_params_id: '',
    ...
    methods: {
      ...
      change_up:function(id,index){    
          ...
          this.$axios.get(this.Ip+'/BasicTreeInfo/'+this.BTN_type+'/'+this.change_params_id+'/'+window.sessionStorage.getItem('admin')+'/'+1)
            .then(res => {
              ...
              if(this.tabledata1!=""&&this.tabledata1!=0){
									for(var z = 0;z<this.tabledata1.length;z++){
				
										this.tabledata1[z].job_status_name=""
				
										if(this.tabledata1[z].job_status == 1){
				
											this.tabledata1[z].job_status_name ="初始狀態，待作業"
				
										}else if(this.tabledata1[z].job_status == 2){
				
											this.tabledata1[z].job_status_name ="作業完了，待翻譯"
				
										}else if(this.tabledata1[z].job_status == 3){
				
											this.tabledata1[z].job_status_name ="翻譯完了"
				
										}
				
				
										for(var j = 0;j< this.tabledata1[z].analysis_list.length;j++){
											this.tabledata1[z].analysis_list[j].job_status_name=""
				
											if(this.tabledata1[z].analysis_list[j].job_status == 1){
				
												this.tabledata1[z].analysis_list[j].job_status_name ="初始狀態，待作業"
				
											}else if(this.tabledata1[z].analysis_list[j].job_status == 2){
				
												this.tabledata1[z].analysis_list[j].job_status_name ="作業完了，待翻譯"
				
											}else if(this.tabledata1[z].analysis_list[j].job_status == 3){
				
												this.tabledata1[z].analysis_list[j].job_status_name="翻譯完了"
	    ...
	    editClick2(definition_id,def_index) {
	      this.select_definition_id = definition_id;
	      this.HU_flag = true;
	      var this_ = this;
	      $(".dialog_html").animate({height: 785}, 800, function () {
	        $(".tagl_div").children(".tagltable").eq(def_index).css({display: "block"})
	      })
	    },
	    editClick3(definition_id,def_index) {
	      this.select_definition_id = definition_id;
	      this.HU_flag = true;
	      var this_ = this;
	      $(".dialog_html").animate({height: 785}, 800, function () {
	        $(".tagl_div").children(".tagltable").eq(def_index).css({display: "block"})
	        $(".tagl_div").eq(def_index).children(".taglana_div").eq(0).children(".taglana_table").eq(0).css({display: "block"})
	      })
	    },
...
<style scoped>
  ...
  .tagltable,.tagl_table,.ana_table{
	height: 169px;
  }
</style>
