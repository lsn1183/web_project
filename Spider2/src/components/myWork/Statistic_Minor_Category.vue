			...
			<div id="Statistic_Function_header">
				<table border="1" style="border-collapse: collapse;text-align: center;width: 100%;border: solid 1px #dfe6ec;font-size: 14px;">
					<tr style="background-color:#eef1f6;width: 100%;">
						<th style="width: 10%;min-width: 100px;borderRight: none;">大分类</th>
						<th style="width: 90%;borderLeft: none;">
							<table border="0" style="border-collapse: collapse;text-align: center;width: 100%;max-height: 60px;">
								<tr>
									<td style="width: 10%;border:solid 1px #dfe6ec;max-width:120px;min-width:120px;" rowspan="2">小分类</td>
			...
			<div id="Statistic_Function_body">
				<table border="1" style="border-collapse: collapse;text-align: center;width: 100%;height: 60px;border: solid 1px #dfe6ec;font-size: 14px;border-bottom: none;" >
					<tr v-for="item in list_Category" class="tr1_hover">
						<td style="width: 10%;overflow:hidden;text-overflow: ellipsis;min-width: 100px;border-right: none;">{{item.major_category}}</td>
						<td style="width: 90%;border: none;">
							<table border="0" style="border-collapse: collapse;text-align: center;width: 100%;height: 100%;">
								<tr v-for="item_kids_kids in item.group_list" style="width: 100%;">
									<td style="width: 10%;height:30px;border:solid 1px #dfe6ec;max-width:120px;min-width:120px;overflow: hidden;text-overflow: ellipsis;white-space: nowrap;" :title="item_kids_kids.small_category">{{item_kids_kids.small_category}}</td>
		...
		export default{
		    ...
		    methods: {
		    	getTable(val){
		    		...
		    		if (this.date_value =='') {
		    			this.$axios.get(this.Ip+"/SummarizeByCategory" )	
						.then(res =>{
							if(res.data.result == 'OK'){
								this.list_Category = res.data.content
								for(var i=0;i<this.list_Category.length;i++){
									
									for(var j = 0;j<this.list_Category[i].group_list.length;j++){
											if(this.list_Category[i].group_list[j].all_arl_num == null){
												this.list_Category[i].group_list[j].all_arl_num= 0								
											}
											if(this.list_Category[i].group_list[j].all_definition_num == null){
												this.list_Category[i].group_list[j].all_definition_num= 0							
											}
											if(this.list_Category[i].group_list[j].all_hu_num == null){
												this.list_Category[i].group_list[j].all_hu_num= 0								
											}
											if(this.list_Category[i].group_list[j].analysis_num == null){
												this.list_Category[i].group_list[j].analysis_num= 0							
											}
											if(this.list_Category[i].group_list[j].definition_hu_num == null){
												this.list_Category[i].group_list[j].definition_hu_num= 0						
											}
											if(this.list_Category[i].group_list[j].hu_arl_num == null){
												this.list_Category[i].group_list[j].hu_arl_num= 0						
											}
											if(this.list_Category[i].group_list[j].analysis_def_num == null){
												this.list_Category[i].group_list[j].analysis_def_num= 0						
											}
											
											if(this.list_Category[i].group_list[j].hu_plan_num == null){
												this.list_Category[i].group_list[j].hu_plan_num= 0						
											}
											if(this.list_Category[i].group_list[j].def_plan_num == null){
												this.list_Category[i].group_list[j].def_plan_num= 0						
											}
											if(this.list_Category[i].group_list[j].analysis_plan_num == null){
												this.list_Category[i].group_list[j].analysis_plan_num= 0						
											}
											this.ARL_Sum += this.list_Category[i].group_list[j].all_arl_num
											this.HUDef_ARL_Sum +=this.list_Category[i].group_list[j].hu_arl_num
											this.HUDef_Sum += this.list_Category[i].group_list[j].all_hu_num
											this.TAGL_ARL_Sum += this.list_Category[i].group_list[j].definition_hu_num
											this.TAGLDef_Sum += this.list_Category[i].group_list[j].all_definition_num
											this.TAGLAna_ARL_Sum += this.list_Category[i].group_list[j].analysis_def_num
											this.TAGLAna_Sum += this.list_Category[i].group_list[j].analysis_num
											
											this.HUDef_Plan_Sum += this.list_Category[i].group_list[j].hu_plan_num
											this.TAGLDef_Plan_Sum += this.list_Category[i].group_list[j].def_plan_num
											this.TAGLAna_Plan_Sum += this.list_Category[i].group_list[j].analysis_plan_num
		    			...
		    			this.date_value = val
		    			this.$axios.get(this.Ip+"/SummarizeByCategory/" + this.date_value)	
						.then(res =>{
							if(res.data.result == 'OK'){
								this.list_Category = res.data.content
								for(var i=0;i<this.list_Category.length;i++){
