	...
	<div class="g_dialog_shadow" v-show="show">
			<div class="g_dialog g_dialog_ex">
				...
				<div class="g_dialog_body g_dialog_body_ex">
					<div class="" v-for="(point_info, index) in hu_data_copy.point_list">
						<h4>
...
//				if(this.hu_data_copy.point_list.length==0){
//					this.hu_data_copy.point_list.unshift({"review_result": '','pointout_no': '','pointout_status': '','pointout_comment': '','reader_check': '',
//			             'reader2_check': '','final_check': '','pointout_charger': '','pointout_priority': '','pointout_date': '','fixed': '','suntec_remark': ''})
//				}else{
//					this.hu_data_copy.point_list.unshift(this.hu_data_copy.point_list[0])
//				}
				this.hu_opened_data = JSON.stringify(this.hu_data_copy.point_list)
				this.show = true;
			},
			close() {
				this.hu_closed_data = JSON.stringify(this.hu_data_copy.point_list)
			...
			save() {
				for(let i = 0;i<this.hu_data_copy.point_list.length;i++){
					if(this.hu_data_copy.point_list[i].suntec_remark == ''){
						if(this.hu_data_copy.point_list[i].suntec_status == '' && this.hu_data_copy.point_list[i].fixed == ''){
							this.$notify({
								type:'error',
