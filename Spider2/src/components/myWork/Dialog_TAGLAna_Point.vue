<template>
	<div class="g_dialog_shadow" v-show="show">
			...
			<div class="g_dialog_body g_dialog_body_ex">
				<div class="" v-for="(point_info, index) in tagl_ana_data_copy.point_list">
					<h4>
					...
					<div style="width: 45%;height: 630px;float: left;border: solid 1px #bfcbd9;border-left: none;">
						...
						<p class="s_section">
							...
							<el-select v-model="point_info.suntec_status" placeholder="请选择" class="s_right s_select" size='small'>
...
//				if(this.tagl_ana_data_copy.point_list.length==0){
//					this.tagl_ana_data_copy.point_list.unshift({"review_result": '','pointout_no': '','pointout_status': '','pointout_comment': '','reader_check': '',
//			             'reader2_check': '','final_check': '','pointout_charger': '','pointout_priority': '','pointout_date': '','fixed': '','suntec_remark': ''})
//				}else{
//					this.tagl_ana_data_copy.point_list.unshift(this.tagl_ana_data_copy.point_list[0])
//				}
				this.tagl_ana_opened_data = JSON.stringify(this.tagl_ana_data_copy.point_list)
				this.show = true;
			},
			close() {
				this.tagl_ana_closed_data = JSON.stringify(this.tagl_ana_data_copy.point_list)
			...
			save() {
				for(let i = 0;i<this.tagl_ana_data_copy.point_list.length;i++){
					if(this.tagl_ana_data_copy.point_list[i].suntec_remark == ''){
						if(this.tagl_ana_data_copy.point_list[i].suntec_status == '' && this.tagl_ana_data_copy.point_list[i].fixed == ''){
							this.$notify({
								type:'error',
