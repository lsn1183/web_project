  ...
  <div id="ARLlist">
        ...
        <li class="li_scroll">
          <ul class="ul_scroll">
            <li class="li" v-for="(item,index) in table1"
                @click.stop='change_up(item.analysis_id,index)' :key="index">
              <i class="Asa_i" :title="item.analysis_id"></i>
              <span class="cy span_width"
                    @click.stop='change_up(item.analysis_id,index)'
                    :class="{'active':num == index}" :title="item.analysis_id">
								<span style="marginLeft:42px">{{item.analysis_id}}</span>
							</span>
              ...
              <span class="cz cen" @click="amint(item.analysis_id,index)"><i class="el-icon-d-arrow-left"> 详细</i></span>
            </li>
          </ul>
        </li>
      </ul>
    ...
    <div id="list" v-show="show_flag">
      <div id="Maplist">
        ...
        <el-collapse accordion v-for="(taglana_record,taglana_index) in tabledata1" :key="taglana_index">
              ...
              <div class="ana_table">
                ...
                <div class='table_content' style="width:7%;border:0 none">
                  ...
                  <p class="table_msg" title="">
                    <el-button type="text" class="table_move" @click="editClick3()" disabled>
...
<script>
  import dialog_basicreq_taglana from './dialog_basicreq_taglana.vue'

  require('../../assets/js/jquery-1.8.3.min.js')
  export default {
    components: {
      hu_dialog: dialog_basicreq_taglana
    },
    data() {
      return {
        change_params_id: '',
      ...
      change_up:function(id,index){    
              ...
              if(this.tabledata1!=""&&this.tabledata1!=0){
									for(var z = 0;z<this.tabledata1.length;z++){
										...
										for(var j = 0;j< this.tabledata1.length;j++){
											this.tabledata1[j].job_status_name=""
				
											if(this.tabledata1[j].job_status == 1){
				
												this.tabledata1[j].job_status_name ="初始狀態，待作業"
				
											}else if(this.tabledata1[j].job_status == 2){
				
												this.tabledata1[j].job_status_name ="作業完了，待翻譯"
				
											}else if(this.tabledata1[j].job_status == 3){
				
												this.tabledata1[j].job_status_name="翻譯完了"
	    ...
	    editClick3() {
	      ...
	      $(".dialog_html").animate({height: 785}, 800, function () {
	        $(".taglana_div").eq(0).children(".taglana_table").eq(0).css({display: "block"})
	      })
	    },
