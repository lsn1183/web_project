    ...
    if request.method == 'GET':
        ...
        try:
            if docType=="1":
                ...
                sql_1="SELECT spec_id, spec_num, spec_name FROM spec_specification"
                c.execute(sql_1)
                tmpList1=[]
                for i in c.fetchall():
                    if i:
...
def search(request):
    if request.method == 'GET':
        ...
        try:
            if searchType=="1":
                sql_1="SELECT t1.req_spec, t1.req_spec_no, t1.version, t1.req_spec_name, t1.req_spec_file_name, t3.spec_chapter_num, t3.spec_chapter_title FROM spec.req_spec_info AS t1 LEFT JOIN spec.req_spec_rel AS t2 ON t1.req_spec_id = t2.req_spec_id LEFT JOIN spec.seq_spec_record AS t3 ON t2.req_rc_id = t3.req_rc_id WHERE t3.spec_chapter_num LIKE '%s' OR t3.spec_chapter_title LIKE '%s'"%(searchKey+"%",searchKey+"%")
                c.execute(sql_1)
                if c.rowcount > 0 :
                    for i in c.fetchall():
                        tmpList1={"req_spec":i[0], "req_spec_no":i[1], "version":i[2], "req_spec_name":i[3], "req_spec_file_name":i[4], "spec_chapter_num":i[5], "spec_chapter_title":i[6]}
                ...
                sql_1="SELECT parent_chapter_num, id, parsed_content FROM spec.spec_functions WHERE parsed_content LIKE '%s' AND id IS NOT NULL ORDER BY parent_chapter_num, id::INTEGER"%("%"+searchKey+"%")
                c.execute(sql_1)
                if c.rowcount > 0 :
                    for i in c.fetchall():
                        tmpList1={"parent_chapter_num":i[0], "id":i[1], "parsed_content":i[2]}
                        record.append(tmpList1)
        finally:
            c.close()
        return HttpResponse(json.dumps(record))
