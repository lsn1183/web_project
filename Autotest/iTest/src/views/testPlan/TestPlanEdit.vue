                            ...
                            <div class="wrapper-content-form-content wrapper-content-form-content-ex">
                                <el-form :model="projForm" :rules="rules" ref="test_plan_form" label-width="110px">
                                    <el-form-item label="测试计划名称" prop="title">
                                        <el-input v-model="projForm.title" class="input-wid" disabled></el-input>
                                    </el-form-item>
...
import { get_one_test_plan, edit_one_test_plan } from '@/api/testPlan'
...
export default {
    ...
    methods: {
        ...
        transfer_req_data_to_display_data() {
            let id = this.$store.getters.operation_data
            const one_test_plan_promise = new Promise((resolve, reject) => {
                get_one_test_plan(id)
                    .then(res => {
                        this.projForm = res.data.result[0]
                        resolve()
                    })
                    .catch(err => {
                        reject(err)
            ...
            const all_case_promise = new Promise((resolve, reject) => {
                get_all_case_no_paging(this.proj_id)
                    .then(res => {
            ...
            Promise.all([one_test_plan_promise, all_case_promise])
                .then(() => {
        ...
        search_detail(table_name) {
            ...
            if(this.case_select_flag = true) { // 在 过滤情况下 this.projForm.case_list 不变化
                
            } else { // 在 没有过滤情况下 this.projForm.case_list 不变化
            ...
            this.$nextTick(function() {
                this.$refs[form_name].validate(valid => {
                    if (valid) {
                        ...
                        const id = this.$store.getters.operation_data
                        edit_one_test_plan(id, data).then(() => {
                            this.$message({
