				...
				snippet += `
                        <h4>
                            <div id="spec_file_info" class="dropdown">
                                <span id="drop"  class="dropdown-toggle" data-toggle="dropdown">${translation2['version']}</span>
                        ...
                        <h4><div id="spec_file_info">${translation2['spec_file_name']}</div><span>:</span>${jsonDT['data']['spec_file_name']}</h4>
                        <h4><div id="spec_file_info">${translation2['language']}</div><span>:</span>${jsonDT['data']['language']}</h4>
                        `;
...
function go2specificationTab(el) {
	...
	t.load_all(function() {
		...
		$('#catalogTree').on("search.jstree", function(e, search_result_data) {
			for (let ii = 0; ii < search_result_data.res.length; ii++) {
				result_text = search_result_data.instance.get_node(search_result_data.res[ii]).text
				if (result_text.search(spec_chapter_num) == 0){
					$("#specificationTab").trigger("click");
					$("#" + search_result_data.res[ii] + " a.jstree-anchor").trigger("click");
					break;
				}
			}
		});
