...
import '@/components/Plugins/style/handsontable.6.less';
...
export default {
    ...
    methods: {
        ...
        created_table(table_all_data) {
            ...
            4.colWidths：设置每一列宽度
             */
            let header_matchup = this.func.header_matchup_data6(),//表6配置数据
        ...
        setHeader(columns_arr, avg_widtn) {
            this.settings.columns = columns_arr.map((item,index) => {
                if (item.header == "View Model") {
                    ...
                    if (item.value == "status_change_chapter"||item.value == "status_change_sub_chapter"||item.value == "status_change_no") {
            ...
            this.settings.nestedHeaders = [
                [this.type,{label:'Action on Status change',colspan:columns_arr.length-1}],
                [this.type,'1',{ label: "Normal", colspan: 3 },{ label: "Forward from", colspan: 4 },
                   { label: 'Back from', colspan: 4 },
                   { label: 'Interrupt', colspan: 4 },"",
                   "UUID",'Event',{label:'Forward from',colspan:5},{label:"Back from",colspan:5},
                    {label:"Interrupt",colspan:5}
                ],
                [this.type,'1',{ label: "Normal", colspan: 3 },'Formula',{ label: "Condition", colspan: 2 },'Action','Formula',
                   { label: 'Condition', colspan: 2 },'Action','Formula',
                   { label: 'Condition', colspan: 2 },'Action',"",'UUID','Event',
