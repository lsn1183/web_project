...
import '@/components/Plugins/style/handsontable.7.less';
...
export default {
    ...
    methods: {
        ...
        created_table(table_all_data) {
            ...
            4.colWidths：设置每一列宽度
             */
            let header_matchup = this.func.header_matchup_data7(),//表5配置数据
        ...
        setHeader(columns_arr, avg_widtn) {
            this.settings.columns = columns_arr.map((item,index) => {
                if (item.header == "View Model") {
                    // console.log(item.value,'进来了')
                    return { data: item.value, type: "autocomplete", source: this.select_list,strict: false, 
                        filter: false, trimDropdown: false, }
                } else {
                    if (item.value == "transition_change_chapter"||item.value == "transition_change_sub_chapter"||item.value == "transition_change_no") {
            ...
            this.settings.nestedHeaders = [
                [this.type,{label:'Action on Transition',colspan:columns_arr.length-1}],
                [this.type,'1',{ label: "Normal", colspan: 3 },{ label: "Back to Other Screen", colspan: 4 },
                   { label: 'Forward to Other Screen', colspan: 4 },
                   { label: 'Interrupt Transition', colspan: 4 },"",
                   "UUID",'Event',{label:'Back to Other Screen',colspan:5},{label:"Back from",colspan:5},
                    {label:"Forward to Other Screen",colspan:5},
                    {label:"Interrupt Transition",colspan:5}
                ],
                [this.type,'1',{ label: "Normal", colspan: 3 },
                    'Formula',{ label: "Condition", colspan: 2 },'Action',
                    'Formula',{ label: 'Condition', colspan: 2 },'Action',
                    'Formula', { label: 'Condition', colspan: 2 },'Action',
                    "",
                    'UUID','Event',
                    {label:'View Model',colspan:2},'Func of Model','Observer','Reply',
                    {label:"View Model",colspan:2},'Func of Model','Observer','Reply',
                    {label:"View Model",colspan:2},'Func of Model','Observer','Reply',
                ],
            ];
            // console.log(this.settings.nestedHeaders[2].length,'this.settings.nestedHeaders');
        ...
        setMethods() {
            ...
            this.settings.afterOnCellMouseDown = (event, coords, TD) => {//点击表格获取相应位置方法
                ...
                if (MouseDown_req_flag) {
                    ...
                    if (type_name == "transition_ope_type") {
                        req_fn.select_opeType_fn(this.proj_id)
                    }else if(type_name == "transition_condition_phrase"){
                        req_fn.select_condition_fn(this.proj_id)
                    }else if(type_name == "transition_action"){
