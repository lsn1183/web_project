...
import '@/components/Plugins/style/handsontable.8.less';
...
export default {
    ...
    methods: {
        ...
        created_table(table_all_data) {
            ...
            4.colWidths：设置每一列宽度
             */
            let header_matchup = this.func.header_matchup_data8(),//表8配置数据
        ...
        setHeader(columns_arr, avg_widtn) {
            this.settings.columns = columns_arr.map((item,index) => {
                if (item.header == "View Model") {
                    ...
                    if (item.value == "trig_change_chapter"||item.value == "trig_change_sub_chapter"||item.value == "trig_change_no") {
            ...
            this.settings.nestedHeaders = [
                [this.type,{label:'Trigger Action',colspan:columns_arr.length-1}],
                [this.type,'1',{ label: "Normal", colspan: 3 },{ label: "Formula", colspan: 1 },
                   { label: 'Condition of Action', colspan: 2 },
                   { label: 'Action in Such Condition', colspan: 1 },
                   'Transition','Sound','Timer','Remark',
                   "",
                   "UUID",'Trigger','ONSID',{label:'View Model',colspan:2},'Func of Model','Observer','Reply','TransType','TransID'
                ],
