					...
					<div class="first" @keyup.enter="SearchComit">
						<el-input :placeholder=placeholderContent v-model="search_id" style="  width:410px">
							<el-select v-model="search_type" slot="prepend" placeholder="请选择类型" style="width:140px" @change="SetPlaceholder">
								<el-option label="ARL" value="arl"></el-option>
								<el-option label="HU要件定義" value="hu"></el-option>
								<el-option label="TAGL要件定義" value="definition"></el-option>
								<el-option label="TAGL要件分析" value="analysis"></el-option>
								<el-option label="HU基本要件定義" value="basic_req_hu"></el-option>
								<el-option label="TAGL基本要件定義" value="basic_req_definition"></el-option>
								<el-option label="TAGL基本要件分析" value="basic_req_analysis"></el-option>
							</el-select>
						...
						for(var i=0;i<this.tableData.length;i++){
							...
							for(var j=0;j<this.tableData[i].classify.length;j++){
								if(this.tableData[i].classify[j]=="arl"){
									...
									str = str+"TAGL要件分析;"
								}else if(this.tableData[i].classify[j]=="basic_req_hu"){
                  							str = str+"HU基本要件定义;"
                						}else if(this.tableData[i].classify[j]=="basic_req_definition"){
                  							str = str+"TAGL基本要件定义;"
                						}else if(this.tableData[i].classify[j]=="basic_req_analysis"){
                  							str = str+"TAGL基本要件分析;"
                						}
