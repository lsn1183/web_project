<template>
	<div id='USERs'>
		...
		<div class="left_menu left_menu_ex fl">
			<div>
				<ul>
					<li :class="{'active': hl==1}"  @click='VIP()' >开发状态</li>
				</ul>
			</div>
		</div>
		<!-- 右边内容 -->				
		<div class="right">
			<div class="sumbit_table sumbit_tabke_ex">
				<el-table :data="tableData"
					...
					<el-table-column
						...
						<template scope="scope">
							...
							<el-select v-model="scope.row.dev_major_category" placeholder="请选择大分类" >
				...
				<!--<el-button @click="eidt_dev_status()" type="primary">编辑</el-button>-->
		 	...
		 	<div class="table table_ex" v-show="G_flag == true">
		 		<!-- 新增开发状态 -->
		 		<h3 style="padding-left: 60px;padding-bottom: 10px;">新增开发状态</h3>
		 		<el-form :model="dynamicValidateForm[0]" ref="dynamicValidateForm" label-width="80px" class="demo-dynamic">
		 			<div style="overflow-y: scroll;max-height: 500px;margin-bottom: 5px;">
		 				<el-form-item
			 				...
			 				>
			 				<el-select v-model="dynamicValidateForm[0].dev_major_category" placeholder="请选择大分类" style="width: 235px;">
	     						<el-option v-for="item in array_detail" :key="item.value" :label="item.value" :value="item.value"></el-option>
	     					</el-select>
			 			</el-form-item>
			 			...
			 			<!--<el-form-item
			 				...
			 				>
			 				<el-input v-model="dynamicValidateForm[0].category" placeholder="请选择中分类"></el-input>
			 			</el-form-item>-->
...
//							}
				...
				let data_change = {classify:"req",status_list:Array_req_dev_status}
			...
			get_HmiDevStatus(){
				this.$axios.get(this.Ip + '/HmiDevStatus/req')
				.then(res => {
					console.log(res)
			...
			submitForm(formName){
				this.$refs[formName].validate( (res) => {
					if(res){
						...
						let data_add = {classify:"req",status_list:Array_add_dev_state}
