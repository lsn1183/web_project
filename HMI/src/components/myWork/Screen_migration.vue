<template>
	<div id='USERs'>
		...
		<div class="left_menu left_menu_ex fl">
			<div>
				<ul>
					<li :class="{'active': hl==1}"  @click='VIP()' >后续画面是否可以迁移</li>
				</ul>
			</div>
		</div>
		<!-- 右边内容 -->				
		<div class="right">
			<div class="sumbit_table sumbit_tabke_ex">
				<el-table :data="tableData"
					...
					<el-table-column
						...
						<template scope="scope">
							<el-select v-model="scope.row.category" placeholder="请选择统计分类" >
				...
				<!-- 	<el-table-column
						...
						>
						<template scope="scope">
							<div v-for="(item,index) in scope.row.detail" style="display: flex;margin:10px 0 ;">
								<el-input v-model="scope.row.detail[index]"  size="small" style="margin-right: 10px;"></el-input>
								<el-button @click="del_detail(scope.row,index)"  size="small">删除</el-button>
							</div>
							<el-button @click="add_detail(scope.row)"  style="float: right;" size="small" >新增</el-button>
						</template>
					</el-table-column> -->
				</el-table>
		 	...
		 	<div class="table table_ex" v-show="G_flag == true">
		 		<!-- 新增开发状态 -->
		 		<h3 style="padding-left: 60px;padding-bottom: 10px;">新增画面迁移状态</h3>
		 		<el-form :model="dynamicValidateForm[0]" ref="dynamicValidateForm" label-width="80px" class="demo-dynamic">
		 			<div style="overflow-y: scroll;max-height: 500px;margin-bottom: 5px;">
		 				<el-form-item
			 				...
			 				>
			 				<el-select v-model="dynamicValidateForm[0].category" placeholder="请选择统计分类" style="width: 235px;">
	     						<el-option v-for="item in array_detail" :key="item.value" :label="item.value" :value="item.value"></el-option>
	     					</el-select>
			 			</el-form-item>
		 				...
		 				<el-form-item
			 				...
			 				>
			 				<el-input v-model="dynamicValidateForm[0].status" placeholder="请选择后续画面开发分类"></el-input>
			 			</el-form-item>
			 			<!-- <el-form-item
			 				v-for="(domain,index) in dynamicValidateForm[0].detail"
			 				:label="'详细'+(index+1)"
			 				:key="domain.key"
			 				:prop="'detail.'+index+'.dev_major_category'"
			 				:rules="{
			 						required: true,message: '内容不能为空',trigger:'blur'
			 					}"
			 				>	
			 				<el-input v-model="domain.dev_major_category" placeholder="详细"></el-input>
...
//							}
				...
				let data_json = {classify:"followup_screen",status_list:Array_req_dev_status}
				this.$axios.post(this.Ip + '/UpdateDevStatus',data_json)
						.then(res =>{
							console.log(res)
			...
			get_HmiDevStatus(){
				this.$axios.get(this.Ip + '/HmiDevStatus/followup_screen')
				.then(res => {
					console.log(res)
			...
			submitForm(formName){
				this.$refs[formName].validate( (res) => {
					if(res){
						...
						let data_json = {classify:"followup_screen",status_list:Array_add_dev_state} 
						this.$axios.post(this.Ip + '/AddDevStatus',data_json)
						.then(res =>{
							this.get_HmiDevStatus()
