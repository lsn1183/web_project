			...
			<el-table border style="top: 10px;height: 100%;width:100%;" :data="tableData" @selection-change="select_line" max-height="700" >
				...
				<el-table-column label="问题类型" header-align="center" width="70">
					<template scope="scope">
						...
						<el-input type="textarea" :title="scope.row.bug_type" v-model="scope.row.bug_type" v-if="scope.row.seen" size="medium"></el-input>
						<span class="text_show" :title="scope.row.bug_type" v-else>{{ scope.row.bug_type }}</span>
					</template>
				</el-table-column>
				<el-table-column label="问题关键字" header-align="center" width="120">
					<template scope="scope">
						<el-input type="textarea" :title='scope.row.bug_keyword' v-model="scope.row.bug_keyword" v-if="scope.row.seen" size="medium"></el-input>
						<span class="text_show" :title='scope.row.bug_keyword' v-else>{{ scope.row.bug_keyword }}</span>
					</template>
				</el-table-column>
				<el-table-column label="问题ID" header-align="center">
					<template scope="scope">
						<el-input type="textarea" v-model="scope.row.bug_id" v-if="scope.row.seen" size="medium"></el-input>
						<span class="text_show" v-else>{{ scope.row.bug_id}}</span>
					</template>
				</el-table-column>
	...
	export default {
		...
		methods: {
			get_table(page,page_size) {
				...
				this.$axios.get(this.Ip + "/HmiExternalBug/"+page+'/'+page_size).then(res => {
        ...
        if(this.Edit_flag == false) {
		        ...
		        if(this.selection_val.length == 0) {
		          ...
		          this.$confirm("是否删除？", '提示', {
		            ...
		            //            console.log(Del_data, "post-data")
			            this.$axios.post(this.Ip + "/HmiExternalBugDel", Del_data).then(res => {
			...
			save_table(row) {
				...
				//				console.log(update_tableData, "end_data")
				//		发送数据给后台
			},
		}
	}
